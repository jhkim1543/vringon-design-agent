...
export function buildBoardModel(st: RunState): BoardModel {
  ...
  if (top.length) {
    ...
    top.forEach((d) => {
      ...
      ;(d.clips ?? []).forEach((c, k) => {
        const id = `clip-${d.spec.design_id}-${k}`
        nodes.push({
          id, kind: 'selection', column: 7, row: conceptRow++,
          title: `${d.spec.design_id} · Concept clip`,
          body: [
            c.backend === 'comfyui'
              ? 'Generated by a local open-source image-to-video model.'
              : 'Camera move built from the still. No video model was running.',
            ...(c.note ? [c.note] : []),
          ],
          clipUrl: c.url,
        })
        edges.push({ from: 'top', to: id, label: 'clip' })
      })
      frames.forEach((fr, k) => {
