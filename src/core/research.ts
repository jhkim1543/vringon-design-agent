...
export function lineForServer(raw?: FootwearLineProfile, itemType?: string) {
  ...
  return {
    ...
    season: lp.product.season, climate: lp.product.climate, environment: lp.product.environment,
    lastFamily: lp.lastFit.lastFamily, toeShape: lp.lastFit.toeShape, toeVolume: lp.lastFit.toeVolume,
    upperOuter: lp.upper.outer, lining: lp.upper.lining, reinforcement: lp.upper.reinforcement, closure: lp.upper.closure, protection: lp.upper.protection,
    ...
    cushioning: lp.performance.cushioning, stability: lp.performance.stability, wetGrip: lp.performance.wetGrip, weightTargetG: lp.performance.weightTargetG,
...
export function toCompetitors(r: CompetitorResearch, priceMin: number, priceMax: number): CompetitorProduct[] {
  ...
  return r.products.map((p, i) => ({
    ...
    purchase_drivers: (p as { purchase_drivers?: string[] }).purchase_drivers,
    design_traits: p.design_traits,
...
export function toSignals(r: TrendResearch): Signal[] {
  return r.signals
    ...
    part: (s as { part?: Signal['part'] }).part,
    social_platforms: (s as { social_platforms?: string[] }).social_platforms?.filter(Boolean),
    mentioned_with: (s as { mentioned_with?: string[] }).mentioned_with?.filter(Boolean),
    purchase_drivers: (s as { purchase_drivers?: string[] }).purchase_drivers?.filter(Boolean),
    price_bands: [],
